...
export function NewNoteCard({ onNoteCreated }: NewNoteProps) {
  ...
  function handleStartEditor(show: boolean) {
  ...
  function handleSaveNote(event: FormEvent) {
    ...
    console.log(content);
    if (content === "") {
      toast.error("Nota nao criada, tente novamente.")
      return;
    }
  ...
  function handleStartRecording() {
    ...
    speechRecognition.onresult = (event:any) => {
      const transcription = Array.from(event.results).reduce((text:any, result:any) => {
        return text.concat(result[0].transcript);
      }, "");

      setContent(transcription as string);
    };
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none">
          <Dialog.Close className="absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100">
            <X className="size-5" onClick={() => handleStartEditor(true)} />
          </Dialog.Close>
